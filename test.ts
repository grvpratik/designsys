...
const getNextRpcEndpoint = () => {
	currentEndpointIndex =
		(currentEndpointIndex + 1) % SOLANA_RPC_ENDPOINTS.length;
...
const generateMockTraderData = (address: string, index: number): Trader => {
	...
	const platform = platforms[(index + addressHash) % platforms.length] as
		| "raydium"
		| "jupiter"
		| "orca"
		| "serum"
...
const generateMockTransactions = (address: string, seed: number = 0): any[] => {
	...
	for (let i = 0; i < txCount; i++) {
		...
		transactions.push({
			signature: `mock-${address.substring(0, 6)}-${i}-${(seed + i).toString(
				36
			)}`,
			blockTime,
...
const fetchWithFallback = async <T>(
	...
	while (retries < maxRetries) {
		try {
			...
			lastError = error;
			console.error(
				`RPC request failed (attempt ${retries + 1}/${maxRetries}):`,
				error
			);
			...
			if (
				...
				console.log(
					"Authentication or rate limit error detected, switching endpoints immediately"
				);
				consecutiveErrors++;
			} else {
				consecutiveErrors++;
...
export const fetchActiveTradingWallets = async (): Promise<string[]> => {
	...
	try {
		...
		try {
			...
			console.log(
				`Found ${signatures.length} transactions for program ${programId}`
			);
				...
				try {
					...
					if (tx?.transaction?.message?.accountKeys) {
						...
						tx.transaction.message.accountKeys
							...
							.forEach((account) =>
								activeWallets.add(account.pubkey.toString())
							);
					}
				} catch (err) {
		...
		if (activeWallets.size > 0) {
			...
			SAMPLE_TRADER_ADDRESSES.slice(0, 3).forEach((addr) =>
...
export const fetchTradingWallets = async (): Promise<Trader[]> => {
	try {
		...
		const tradersPromises = limitedAddresses.map(async (address, index) => {
			...
			try {
				...
				if (transactions && transactions.length > 0) {
					hasRealData = true;
					const { profit, winRate, tradingVolume, trades, hasPumpFun } =
						calculateTraderMetrics(transactions, address);
					...
					if (trades < 2) {
						// Reduced minimum trade count to show more wallets
		...
		const traders = (await Promise.all(tradersPromises)).filter(
			Boolean
		) as Trader[];
		...
		const additionalTraders = SAMPLE_TRADER_ADDRESSES.filter(
			(addr) => !traders.some((t) => t.address === addr)
		)
			.slice(0, Math.max(0, 10 - traders.length))
			.map((address, index) =>
				generateMockTraderData(address, index + traders.length)
			);
	...
	try {
		...
		if (!followedTraders.includes(traderAddress)) {
			...
			localStorage.setItem(
				FOLLOWED_TRADERS_KEY,
				JSON.stringify(followedTraders)
			);
		}
		return true;
	} catch (error) {
...
export const calculateTraderMetrics = (
	transactions: any[],
	address: string
): {
	profit: number;
	...
	for (const tx of transactions) {
		...
		const programIds = tx.details.transaction.message.accountKeys.map(
			(account: any) => account.pubkey?.toString() || ""
		);
		...
		if (isDexTransaction) {
			...
			if (tx.status === "Success") {
				...
				if (
					tx.details.meta &&
					tx.details.meta.postBalances &&
					tx.details.meta.preBalances
				) {
					...
					if (index !== -1) {
						const preBalance =
							tx.details.meta.preBalances[index] / LAMPORTS_PER_SOL;
						const postBalance =
							tx.details.meta.postBalances[index] / LAMPORTS_PER_SOL;
...
export const generateTradingName = (
	address: string,
	platform: string = "unknown",
	hasPumpFun: boolean = false
): string => {
...
export const determineTradingPlatform = (
	transactions: any[]
): "raydium" | "jupiter" | "orca" | "serum" | "pumpfun" | "unknown" => {
	...
	for (const tx of transactions) {
		...
		if (
			programIds.includes(DEX_PROGRAM_IDS.RAYDIUM_SWAP) ||
			programIds.includes(DEX_PROGRAM_IDS.RAYDIUM_LIQUIDITY)
		) {
	...
	const platform = Object.entries(platformCounts).sort(
		(a, b) => b[1] - a[1]
	)[0][0];
	return platform as
		| "raydium"
...
export const fetchRaydiumReadyTokens = async (): Promise<Token[]> => {
	try {
		...
		return SAMPLE_TOKENS.filter(
			(token) => token.bondingCurveProgressValue >= 95
		);
	}
};
...
export const fetchRecentlyListedTokens = async (): Promise<Token[]> => {
	try {
		...
		return SAMPLE_TOKENS.filter(
			(token) => token.isPumpFun && token.isRaydiumReady
		);
	}
};
...
export const fetchWatchedTokens = async (): Promise<Token[]> => {
	try {
		...
		return allTokens.filter((token) =>
			watchedAddresses.includes(token.address)
		);
	} catch (error) {
...
export const checkIfTokenIsOnRaydium = async (
	tokenAddress: string
): Promise<boolean> => {
	try {
		...
		console.error(
			`Error checking if token ${tokenAddress} is on Raydium:`,
			error
		);
		return false;
	}
...
export const getTokensByBondingCurveProgress = async (
	count = 10
): Promise<Token[]> => {
	const allTokens = await fetchPumpFunTokens();
	return allTokens
